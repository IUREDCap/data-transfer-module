...
Feature: Data Transfer with API
  In order to transfer data from one project to another
  As a user
  I need to be able to specify and run data transfers using the API
  ...
  Scenario: Transfer data
    ...
    And I delete configuration "behat-export-basic-api-test" if it exists
    Then I should not see "behat-export-basic-api-test"
    And I should not see "REDCap crashed"

    When I add configuration "behat-export-basic-api-test"
    And I follow configuration "behat-export-basic-api-test"
    And I enable export to API project with URL property "redcap" "api_url" and token property "project_basic_destination" "api_token"
    And I check transfer files
    And I add field mapping "" "ALL" "ALL" to "" "MATCHING" "EQUIVALENT"
    Then I should see "behat-export-basic-api-test"
    But I should not see "REDCap crashed"
